...
function sanitizeMlbLeg(raw){
  ...
  const player=clean(raw.player),market=canonicalMarket(raw.market);if(!player||player.length<2||!/^[A-Za-z][A-Za-z0-9_]{1,63}$/.test(market))return null;
...
function numericLeg(line,re,market){
  ...
  const player=clean(line.slice(0,cut).replace(/[-–—:]\s*$/,''));if(!player||player.length<2)return null;
...
module.exports={MLB_MARKETS,MLB_BINARY,canonicalizeMlbLeg,sanitizeMlbLeg,hasMlbContext,mlbHeuristic,supplementalMlbVision};
