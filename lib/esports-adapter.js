...
function findEsportsFinal(snapshot,leg,{referenceTime,eventRow}={}){
  ...
  if(eventRow){
    ...
    if(exactId&&samePair(exactId,eventRow)&&[exactId.homeTeam,exactId.awayTeam].some(team=>sameTeam(team,leg.player)))return exactId;
...
async function analyzeEsportsSlip(rawLegs,options={}){
  ...
  const trustedResults=results?.settlementConnected===true?results:null;
  ...
  for(const leg of legs){
    ...
    const final=findEsportsFinal(trustedResults,leg,{referenceTime,eventRow});
    if(final){analyzed.push(gradeFinal(leg,final,eventRow));continue;}
    if(!eventRow){
      analyzed.push(unresolved(leg,'esports-match-winner-not-found-in-current-pinnacle-snapshot'));
      continue;
    }
    ...
    if(now>=start){analyzed.push(unresolved(leg,'esports-trustworthy-live-final-source-not-connected',{...base,gameState:'in-or-post'}));continue;}
    const probability=fairProbability(eventRow);
  ...
  return {ok:true,generatedAt:new Date().toISOString(),dataGeneratedAt:latestDataTime(odds?.meta?.fetchedAt,trustedResults?.generatedAt),source:'The Sports Outpost ESPORTS complete two-sided Pinnacle match-winner snapshot',results:analyzed};
}
