...
const SPORT_CONFIG={
  SOCCER:{file:'soccer-odds.json'},
  TENNIS:{file:'tennis-odds.json'},
...
async function loadSnapshot(sport){
  ...
  const response=await fetch(`${OUTPOST_BASE}/${cfg.file}`,{headers:{accept:'application/json','user-agent':'ParlayPing/0.9'},cache:'no-store'});
  if(response.status===404)return {meta:{sport},rows:[]};
  if(!response.ok)throw new Error(`${sport} snapshot request failed (${response.status})`);
  return response.json();
}
function playerRows(snapshot,leg){
  return (snapshot?.rows||[]).filter(row=>normName(row.player)===normName(leg.player)&&(!leg.team||!row.team||teamMatches(leg.team,row.team)));
}
...
async function analyzeExtendedSlip(sport,rawLegs,options={}){
  ...
  const snapshot=await loadSnapshot(sport),referenceTime=options.referenceTime||new Date().toISOString(),now=Number.isFinite(Number(options.now))?Number(options.now):Date.now(),results=[];
  for(const leg of legs){
    ...
    if(now>=start){results.push({...unresolved(leg,`${sport.toLowerCase().replaceAll('_','-')}-live-grading-not-connected`),gameId:String(eventRow.eventId||''),matchup:matchup(eventRow),startTimeUTC:eventRow.commenceTime||null,gameState:'in-or-post'});continue;}
  ...
  return {ok:true,generatedAt:new Date().toISOString(),dataGeneratedAt:snapshot?.meta?.fetchedAt||null,source:`The Sports Outpost ${sport} ParlayAPI sportsbook snapshot`,results};
}

module.exports={SPORT_CONFIG,MARKET_LABELS,BINARY_MARKETS,normName,rowMatchesLine,quoteProbability,displayMarket,targetForLeg,marketOptions,analyzeExtendedSlip};
