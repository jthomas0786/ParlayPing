...
function isMilestoneQuote(row,side){
  const key=String(row?.marketKey||'').toLowerCase();
  if(key.endsWith('_alt'))return true;
  const over=finite(row?.overPrice),under=finite(row?.underPrice);
  return side==='under' ? under!=null&&over==null : over!=null&&under==null;
}
function quoteMatchesLine(row,leg){
  if(MARKET_META[leg.market]?.binary)return Math.abs((finite(row?.line)??0)-0.5)<1e-9;
  const requested=finite(leg.line),offered=finite(row?.line);if(requested==null||offered==null)return false;
  if(!leg.inclusive)return Math.abs(offered-requested)<1e-9;
  if(!Number.isInteger(requested))return Math.abs(offered-requested)<1e-9;
  if(leg.side==='under'){
    if(isMilestoneQuote(row,'under'))return Math.abs(offered-requested)<1e-9;
    return Math.abs(offered-(requested+0.5))<1e-9;
  }
  if(isMilestoneQuote(row,'over'))return Math.abs(offered-requested)<1e-9;
  return Math.abs(offered-(requested-0.5))<1e-9;
}
function matchingOddsRows(odds,match,leg){
  const start=Date.parse(match?.game?.startTime||''),sideTeam=match?.player?.team;
  return oddsPlayerRows(odds,{...leg,team:leg.team||sideTeam}).filter(r=>{
    if(r.market!==leg.market||!quoteMatchesLine(r,leg))return false;
...
function marketOptions(odds,match,leg){
  ...
  for(const [line,rows] of groups){
    ...
    const chosen=priced[0]||rows[0],milestone=isMilestoneQuote(chosen,side);
    const displayLine=milestone?line:(side==='over'&&Number.isInteger(line+0.5)?line+0.5:line);
...
module.exports={SPORT_CONFIG,MARKET_META,parseSummaryPlayers,displayMarket,targetForLeg,conditionMet,settleStatus,quoteMatchesLine,quoteProbability,marketOptions,analyzeBasketballSlip,normName};
