...
const GENERIC_MATCH_SPORTS=new Set(['VOLLEYBALL','CRICKET','RUGBY_LEAGUE','AFL','BOXING']);
const EXTENDED_SPORTS=new Set(['SOCCER','TENNIS','MMA','ESPORTS','TABLE_TENNIS','GOLF',...GENERIC_MATCH_SPORTS]);
const EXTENDED_BINARY=new Set(['anytimeGoal','toReceiveCard','matchWinner','fightWinner','tournamentWinner']);
const EXTENDED_MARKETS={
  ...
  VOLLEYBALL:new Set(['matchWinner']),
  CRICKET:new Set(['matchWinner']),
  RUGBY_LEAGUE:new Set(['matchWinner']),
  AFL:new Set(['matchWinner']),
  BOXING:new Set(['matchWinner']),
  GOLF:new Set(['tournamentWinner'])
};
...
function normalizeSport(value){
  ...
  const aliases={
    UFC:'MMA',MIXEDMARTIALARTS:'MMA',TABLETENNIS:'TABLE_TENNIS',PINGPONG:'TABLE_TENNIS',
    EPL:'SOCCER',PREMIERLEAGUE:'SOCCER',MLS:'SOCCER',LALIGA:'SOCCER',SERIEA:'SOCCER',BUNDESLIGA:'SOCCER',LIGUE1:'SOCCER',
    CS2:'ESPORTS',COUNTERSTRIKE:'ESPORTS',COUNTERSTRIKE2:'ESPORTS',VALORANT:'ESPORTS',LEAGUEOFLEGENDS:'ESPORTS',LOL:'ESPORTS',DOTA:'ESPORTS',DOTA2:'ESPORTS',
    VOLLEYBALL:'VOLLEYBALL',CRICKET:'CRICKET',NRL:'RUGBY_LEAGUE',RUGBYLEAGUE:'RUGBY_LEAGUE',AFL:'AFL',AUSSIERULES:'AFL',AUSTRALIANRULES:'AFL',BOXING:'BOXING',
    GOLF:'GOLF',PGA:'GOLF',DPWORLD:'GOLF',LIVGOLF:'GOLF'
  };
...
function contextSport(text){
  ...
  if(/\b(volleyball)\b/i.test(value))return 'VOLLEYBALL';
  if(/\b(cricket|\bipl\b|twenty20|\bt20\b|one\s+day\s+international|\bodi\b|test\s+match)\b/i.test(value))return 'CRICKET';
  if(/\b(rugby\s+league|\bnrl\b)\b/i.test(value))return 'RUGBY_LEAGUE';
  if(/\b(aussie\s+rules|australian\s+rules|\bafl\b)\b/i.test(value))return 'AFL';
  if(/\bboxing\b/i.test(value))return 'BOXING';
  if(/\b(golf|\bpga\b|dp\s+world|liv\s+golf)\b/i.test(value))return 'GOLF';
...
function canonicalizeLeg(raw){
  ...
  const genericAliases={winner:'matchWinner',moneyline:'matchWinner',ml:'matchWinner',gameWinner:'matchWinner',seriesWinner:'matchWinner'};
  const aliases={
    ...
    ESPORTS:{...genericAliases,kills:'killsMaps12',headshots:'headshotsMaps12',firstBloods:'firstBloodsMaps12'},
    TABLE_TENNIS:genericAliases,
    VOLLEYBALL:genericAliases,
    CRICKET:genericAliases,
    RUGBY_LEAGUE:genericAliases,
    AFL:genericAliases,
    BOXING:genericAliases,
    GOLF:{winner:'tournamentWinner',outright:'tournamentWinner',outrights:'tournamentWinner',tournamentWinner:'tournamentWinner',toWinTournament:'tournamentWinner'}
  };
...
function numeric(line,re,market,sport){
  ...
  const cut=Math.min(marketIndex,numberIndex);const player=clean(line.slice(0,cut).replace(/[-–—:]\s*$/,''));
...
function winnerLeg(line,sport){
  let re;
  if(sport==='MMA')re=/^(.{2,80}?)(?:\s+[-–—:]?\s*)(?:to\s+win(?:\s+the)?\s+fight|fight\s+winner|moneyline|\bml\b)/i;
  else if(sport==='BOXING')re=/^(.{2,80}?)(?:\s+[-–—:]?\s*)(?:to\s+win(?:\s+the)?(?:\s+(?:fight|bout|match))?|fight\s+winner|bout\s+winner|match\s+winner|moneyline|\bml\b)/i;
  else if(sport==='ESPORTS')re=/^(.{2,80}?)(?:\s+[-–—:]?\s*)(?:to\s+win(?:\s+the)?\s+(?:match|series)|match\s+winner|series\s+winner|moneyline|\bml\b)/i;
  else if(['TENNIS','TABLE_TENNIS'].includes(sport))re=/^(.{2,80}?)(?:\s+[-–—:]?\s*)(?:to\s+win(?:\s+the)?\s+match|match\s+winner|moneyline|\bml\b)/i;
  else if(sport==='GOLF')re=/^(.{2,80}?)(?:\s+[-–—:]?\s*)(?:to\s+win(?:\s+the)?(?:\s+(?:tournament|event|open|championship))?|tournament\s+winner|event\s+winner|outright(?:\s+winner)?)/i;
  else if(GENERIC_MATCH_SPORTS.has(sport))re=/^(.{2,80}?)(?:\s+[-–—:]?\s*)(?:to\s+win(?:\s+the)?(?:\s+(?:match|game))?|match\s+winner|game\s+winner|moneyline|\bml\b)/i;
  if(!re)return null;
  const m=line.match(re);if(!m)return null;
  const market=sport==='MMA'?'fightWinner':sport==='GOLF'?'tournamentWinner':'matchWinner';
  return sanitizeExtended({sport,player:m[1],market,side:'yes',line:null,inclusive:true,originalText:line});
}
function extendedHeuristic(text){
  ...
  for(const line of lines){
    ...
    const binary=winnerLeg(line,sport);
    if(binary){out.push(binary);continue;}
...
async function supplementalVision({text,mediaUrls}){
  ...
  const content=[{type:'input_text',text:`Extract only explicit props or winner selections for SOCCER, TENNIS, MMA, ESPORTS, TABLE_TENNIS, VOLLEYBALL, CRICKET, RUGBY_LEAGUE, AFL, BOXING, or GOLF. Use exact canonical markets: SOCCER shots, shotsOnTarget, assists, goalsAssists, fouls, goals, anytimeGoal, toReceiveCard, saves; TENNIS gamesWon, gamesPlayed, setsWon, setsPlayed, aces, doubleFaults, breakPointsWon, tiebreaksPlayed, firstSetAces, matchWinner; MMA fightWinner; ESPORTS matchWinner, killsMaps12, killsMaps123, killsMaps13, map1Kills, map3Kills, map4Kills, map5Kills, headshotsMaps12, map3Headshots, firstBloodsMaps12, points, assists; TABLE_TENNIS, VOLLEYBALL, CRICKET, RUGBY_LEAGUE, AFL, BOXING matchWinner; GOLF tournamentWinner. Winner markets are binary yes with null line. For GOLF, only an explicit golfer tournament/event outright winner selection is allowed; do not extract top-5/top-10, round props, matchup props, or leaderboard position. For ESPORTS and generic match sports, team/competitor moneyline or match winner is allowed. Do not invent a winner selection from a matchup alone. Omit uncertain legs. Post text: ${String(text||'').slice(0,3000)}` }];
  for(const url of mediaUrls.slice(0,4))content.push({type:'input_image',image_url:url,detail:'high'});
  const schema={type:'object',additionalProperties:false,properties:{legs:{type:'array',maxItems:20,items:{type:'object',additionalProperties:false,properties:{sport:{type:'string',enum:['SOCCER','TENNIS','MMA','ESPORTS','TABLE_TENNIS','VOLLEYBALL','CRICKET','RUGBY_LEAGUE','AFL','BOXING','GOLF','OTHER']},player:{type:'string'},team:{type:['string','null']},market:{type:'string'},side:{type:'string',enum:['over','under','yes','no']},line:{type:['number','null']},inclusive:{type:'boolean'},originalText:{type:'string'}},required:['sport','player','team','market','side','line','inclusive','originalText']}}},required:['legs']};
...
async function parseSlip(input={}){
  ...
  let supplemental=[];
  const needsExtendedVision=mediaUrls.length&&!base.visionError&&(!normalized.length||normalized.some(leg=>leg.sport==='OTHER'));
...
module.exports={parseSlip,extendedHeuristic,canonicalizeLeg,sanitizeExtended,normalizeSport,EXTENDED_SPORTS,EXTENDED_MARKETS,EXTENDED_BINARY,GENERIC_MATCH_SPORTS};
