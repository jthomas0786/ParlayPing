...
const EXTENDED_MARKETS={
  ...
  ESPORTS:new Set(['killsMaps12','killsMaps123','killsMaps13','map1Kills','map3Kills','map4Kills','map5Kills','headshotsMaps12','map3Headshots','firstBloodsMaps12','points','assists']),
...
function normalizeSport(value){
  ...
  const aliases={UFC:'MMA',MIXEDMARTIALARTS:'MMA',TABLETENNIS:'TABLE_TENNIS',PINGPONG:'TABLE_TENNIS',EPL:'SOCCER',PREMIERLEAGUE:'SOCCER',MLS:'SOCCER',LALIGA:'SOCCER',SERIEA:'SOCCER',BUNDESLIGA:'SOCCER',LIGUE1:'SOCCER'};
...
function canonicalizeLeg(raw){
  ...
  const aliases={
    ...
    ESPORTS:{kills:'killsMaps12',headshots:'headshotsMaps12',firstBloods:'firstBloodsMaps12'},
...
async function supplementalVision({text,mediaUrls}){
  ...
  const content=[{type:'input_text',text:`Extract only explicit player/fighter props for SOCCER, TENNIS, MMA, ESPORTS, or TABLE_TENNIS. Use exact canonical markets: SOCCER shots, shotsOnTarget, assists, goalsAssists, fouls, goals, anytimeGoal, toReceiveCard, saves; TENNIS gamesWon, gamesPlayed, setsWon, setsPlayed, aces, doubleFaults, breakPointsWon, tiebreaksPlayed, firstSetAces, matchWinner; MMA fightWinner; ESPORTS killsMaps12, killsMaps123, killsMaps13, map1Kills, map3Kills, map4Kills, map5Kills, headshotsMaps12, map3Headshots, firstBloodsMaps12, points, assists; TABLE_TENNIS matchWinner. Player/fighter winner markets are binary yes with null line. Do not treat team/game moneylines as player props. Omit uncertain legs. Post text: ${String(text||'').slice(0,3000)}` }];
