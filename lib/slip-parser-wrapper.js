...
const EXTENDED_SPORTS=new Set(['SOCCER','TENNIS','MMA','ESPORTS','TABLE_TENNIS',...GENERIC_MATCH_SPORTS]);
...
function winnerLeg(line,sport){
  ...
  return sanitizeExtended({sport,player:m[1],market:sport==='MMA'?'fightWinner':'matchWinner',side:'yes',line:null,inclusive:true,originalText:line});
}
...
async function supplementalVision({text,mediaUrls}){
  ...
  const content=[{type:'input_text',text:`Extract only explicit props or winner selections for SOCCER, TENNIS, MMA, ESPORTS, TABLE_TENNIS, VOLLEYBALL, CRICKET, RUGBY_LEAGUE, AFL, or BOXING. Use exact canonical markets: SOCCER shots, shotsOnTarget, assists, goalsAssists, fouls, goals, anytimeGoal, toReceiveCard, saves; TENNIS gamesWon, gamesPlayed, setsWon, setsPlayed, aces, doubleFaults, breakPointsWon, tiebreaksPlayed, firstSetAces, matchWinner; MMA fightWinner; ESPORTS matchWinner, killsMaps12, killsMaps123, killsMaps13, map1Kills, map3Kills, map4Kills, map5Kills, headshotsMaps12, map3Headshots, firstBloodsMaps12, points, assists; TABLE_TENNIS, VOLLEYBALL, CRICKET, RUGBY_LEAGUE, AFL, BOXING matchWinner. Winner markets are binary yes with null line. For ESPORTS and the generic match sports, team/competitor moneyline or match winner is allowed and the selected team/competitor name belongs in player. Do not invent a winner selection from a matchup alone. Omit uncertain legs. Post text: ${String(text||'').slice(0,3000)}` }];
  for(const url of mediaUrls.slice(0,4))content.push({type:'input_image',image_url:url,detail:'high'});
  const schema={type:'object',additionalProperties:false,properties:{legs:{type:'array',maxItems:20,items:{type:'object',additionalProperties:false,properties:{sport:{type:'string',enum:['SOCCER','TENNIS','MMA','ESPORTS','TABLE_TENNIS','VOLLEYBALL','CRICKET','RUGBY_LEAGUE','AFL','BOXING','OTHER']},player:{type:'string'},team:{type:['string','null']},market:{type:'string'},side:{type:'string',enum:['over','under','yes','no']},line:{type:['number','null']},inclusive:{type:'boolean'},originalText:{type:'string'}},required:['sport','player','team','market','side','line','inclusive','originalText']}}},required:['legs']};
