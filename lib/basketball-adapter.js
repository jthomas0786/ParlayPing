...
function exactBookOffers(odds,leg,row){
  const side=leg.side==='under'||leg.side==='no'?'under':'over';
  const offers={};
  for(const candidate of matchingQuoteRows(odds,leg,row)){
    const book=String(candidate?.book||candidate?.sportsbook||'').trim();
    const american=finite(side==='under'?candidate?.underPrice:candidate?.overPrice);
    if(!book||american===null)continue;
    const selectionLink=(side==='under'?candidate?.underLink:candidate?.overLink)||candidate?.selectionLink||candidate?.deepLink||candidate?.link||null;
    const selectionId=(side==='under'?candidate?.underSid:candidate?.overSid)||candidate?.selectionId||candidate?.sid||null;
    const offer={
    ...
    const previous=offers[book];
    if(!previous||american>finite(previous.oddsAmerican))offers[book]=offer;
  }
  const ranked=Object.entries(offers).sort((a,b)=>(finite(b[1]?.oddsAmerican)??-99999)-(finite(a[1]?.oddsAmerican)??-99999));
  const preferred=ranked.length?{sportsbook:ranked[0][0],...ranked[0][1]}:null;
  return {bookOffers:offers,preferred};
}
function marketOptionsSafe(odds,leg,row){
  ...
  for(const [line,rows] of groups){
    ...
    const selectionLink=(side==='under'?chosen?.underLink:chosen?.overLink)||chosen?.selectionLink||chosen?.deepLink||null;
    const selectionId=(side==='under'?chosen?.underSid:chosen?.overSid)||chosen?.selectionId||null;
    out.push({line,probability:clamp(probability),price:finite(side==='under'?chosen.underPrice:chosen.overPrice),book:chosen.book||null,deepLink:selectionLink,selectionLink,selectionId,preserved:Boolean(chosen?.preserved)});
  }
...
function pregameResult(odds,leg,row){
  ...
  const offerBundle=exactBookOffers(odds,leg,row);
  const preferred=offerBundle.preferred;
  return {
    ...leg,
    ...
    bookOffers:offerBundle.bookOffers,
    ...(preferred?{
      oddsAmerican:preferred.oddsAmerican,
      sportsbook:preferred.sportsbook,
      sportsbookLink:preferred.selectionLink||null,
  ...
  if(liveLegs.length){
    try{
      ...
      for(const liveResult of live.results||[]){
        const sourceLeg=legs.find(leg=>String(leg.id||`${leg.player}|${leg.market}|${leg.line}`)===String(liveResult.id||`${liveResult.player}|${liveResult.market}|${liveResult.line}`))||liveResult;
        const row=closestEventRow(odds,sourceLeg,referenceTime);
        const offerBundle=row?exactBookOffers(odds,sourceLeg,row):{bookOffers:{},preferred:null};
        const preferred=offerBundle.preferred;
        results.push({
          ...liveResult,
          bookOffers:{...(liveResult.bookOffers||{}),...offerBundle.bookOffers},
          ...(preferred&&!Number.isFinite(finite(liveResult.oddsAmerican))?{
            oddsAmerican:preferred.oddsAmerican,
            sportsbook:preferred.sportsbook,
            sportsbookLink:preferred.selectionLink||liveResult.sportsbookLink||null,
            sportsbookSelectionId:preferred.selectionId||liveResult.sportsbookSelectionId||null,
            oddsSnapshotTime:preferred.snapshotTime||liveResult.oddsSnapshotTime||null,
            oddsPreserved:Boolean(preferred.preserved)
          }:{})
        });
      }
...
module.exports={analyzeBasketballSlip,closestEventRow,pregameResult,quoteProbabilitySafe,exactBookOffers,marketOptionsSafe,withEspnServerFetch,normPlayerName,repairVisionMarket};