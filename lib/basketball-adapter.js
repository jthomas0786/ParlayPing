...
async function analyzeBasketballSlip(sport,rawLegs,options={}){
  ...
  if(liveLegs.length){
    try{
      const live=await withEspnServerFetch(()=>legacy.analyzeBasketballSlip(sport,liveLegs,options));
...
module.exports={analyzeBasketballSlip,closestEventRow,pregameResult,quoteProbabilitySafe,marketOptionsSafe,withEspnServerFetch};
