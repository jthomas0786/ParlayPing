...
const OUTPOST_BASE=String(process.env.SPORTS_OUTPOST_RAW_BASE||'https://raw.githubusercontent.com/jthomas0786/The-Sports-Outpost/main/slates').replace(/\/$/,'');
...
function normBook(value){
  ...
  const map={draftkings:'DraftKings',draftkingssportsbook:'DraftKings',dk:'DraftKings',fanduel:'FanDuel',fanduelsportsbook:'FanDuel',fd:'FanDuel',bet365:'bet365',caesars:'Caesars',caesarssportsbook:'Caesars',williamhill:'Caesars',betmgm:'BetMGM',mgm:'BetMGM',fanatics:'Fanatics',fanaticssportsbook:'Fanatics',espnbet:'ESPN BET',espn:'ESPN BET',hardrock:'Hard Rock',hardrockbet:'Hard Rock',betrivers:'BetRivers',pinnacle:'Pinnacle',bovada:'Bovada',parx:'Parx',parxcasino:'Parx'};
  return map[key]||raw||null;
}
function quoteSide(leg){return leg?.side==='under'||leg?.side==='no'?'under':'over';}
function sidePrice(row,side){return finite(side==='under'?row?.underPrice:row?.overPrice);}
function sideSelectionLink(row,side){
  const s=side==='under'?'under':'over';
  const upper=s[0].toUpperCase()+s.slice(1);
  const bySide=row?.selectionBySide||row?.selection_by_side||null;
  const links=row?.selectionLinks||row?.links||row?.betslipLinks||row?.betslip_links||null;
  const entry=bySide&&typeof bySide==='object'?bySide[s]:null;
  return row?.[`${s}Link`]||row?.[`${s}_link`]||row?.[`${s}Url`]||row?.[`${s}_url`]||row?.[`link${upper}`]||row?.[`url${upper}`]||entry?.link||entry?.url||(links&&typeof links==='object'?(links[s]||links[s==='over'?'yes':'no']):null)||row?.deepLink||row?.selectionLink||row?.link||row?.url||null;
}
function sideSelectionId(row,side){
  const s=side==='under'?'under':'over';
  const upper=s[0].toUpperCase()+s.slice(1);
  const bySide=row?.selectionBySide||row?.selection_by_side||null;
  const entry=bySide&&typeof bySide==='object'?bySide[s]:null;
  return row?.[`${s}Sid`]||row?.[`${s}_sid`]||row?.[`sid${upper}`]||entry?.sid||entry?.selectionId||entry?.selection_id||null;
}
...
function sportsbookQuotesSafe(odds,leg,row){
  const side=quoteSide(leg);
  const bookOffers={};
...
function marketOptionsSafe(odds,leg,row){
  ...
  for(const [line,rows] of groups){
    ...
    const priced=rows.filter(r=>sidePrice(r,side)!==null).sort((a,b)=>(sidePrice(b,side)??-99999)-(sidePrice(a,side)??-99999));
    const chosen=priced[0]||rows[0];
    out.push({line,probability:clamp(probability),price:sidePrice(chosen,side),book:normBook(chosen.book)||null,deepLink:sideSelectionLink(chosen,side)});
  }
...
module.exports={analyzeBasketballSlip,explicitSlipOffer,closestEventRow,pregameResult,quoteProbabilitySafe,sportsbookQuotesSafe,marketOptionsSafe,withEspnServerFetch,normPlayerName,normBook,sideSelectionLink,repairVisionMarket};