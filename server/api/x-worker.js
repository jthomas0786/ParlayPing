...
function hasCurrentMention(mention){return new RegExp(`@${X_USERNAME}\\b`,'i').test(String(mention?.text||''));}
function isActionableMention(mention,replied,botUserId){if(replied.has(String(mention?.id))||mention?.possibly_sensitive||/\b(stop|unsubscribe|opt\s*out)\b/i.test(mention?.text||''))return false;if(botUserId){if(String(mention?.author_id||'')===String(botUserId))return false;if(!hasCurrentMention(mention))return false;if(!(mention?.referenced_tweets||[]).some(ref=>ref.type==='replied_to'))return false;}return true;}
...
async function probeMentions(){const userId=await resolveBotUserId();const{mentions,replied}=await getContext(userId);const users=new Map((mentions.includes?.users||[]).map(user=>[String(user.id),user]));return(mentions.data||[]).slice(0,20).map(mention=>{const author=users.get(String(mention.author_id));return{mentionId:String(mention.id),authorId:String(mention.author_id||''),authorUsername:author?.username||null,createdAt:mention.created_at||null,conversationId:mention.conversation_id||null,parentId:replyTarget(mention),possiblySensitive:Boolean(mention.possibly_sensitive),alreadyReplied:replied.has(String(mention.id)),explicitMention:hasCurrentMention(mention),eligible:isActionableMention(mention,replied,userId),text:String(mention.text||'').slice(0,500),references:mention.referenced_tweets||[]};});}

async function processMentions({dryRun,idempotencySecret}){
  const userId=await resolveBotUserId();const{mentions,replied}=await getContext(userId);const includesTweets=new Map((mentions.includes?.tweets||[]).map(t=>[String(t.id),t]));const mediaByKey=new Map((mentions.includes?.media||[]).map(m=>[String(m.media_key),m]));const actionableMentions=(mentions.data||[]).filter(mention=>isActionableMention(mention,replied,userId));await hydrateMissingParents(actionableMentions,includesTweets,mediaByKey);
  const candidates=[];
