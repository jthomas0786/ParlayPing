...
function isActionableMention(mention,replied,botUserId,options={}){
  ...
  if(botUserId){
    ...
    if(conversationId&&botConversations?.has(conversationId)){
      ...
      if(String(parent?.author_id||'')===String(botUserId))return true;
      if(!hasVisibleBodyMention(mention?.text,X_USERNAME))return false;
    }
  }
  return true;
}
...
async function processMentions({dryRun,idempotencySecret}){
  ...
  const actionableMentions=[],candidates=[];
  for(const mention of initialMentions){const parentId=replyTarget(mention),parent=parentId?includesTweets.get(String(parentId)):null;if(isActionableMention(mention,replied,userId,{parent,botConversations}))actionableMentions.push(mention);else candidates.push({mentionId:mention.id,parentId:parentId||null,conversationId:mention.conversation_id||null,status:'ignored-inherited-thread-mention'});}
