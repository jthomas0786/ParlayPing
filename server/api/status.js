module.exports = async function handler(req, res) {
  ...
  return res.status(200).json({
    ...
    engine: {
      ready: true,
      sports: ['NFL','NCAAF','MLB','NHL','NBA','NCAAB','WNBA','SOCCER','TENNIS','MMA','ESPORTS','TABLE_TENNIS','VOLLEYBALL','CRICKET','RUGBY_LEAGUE','AFL','BOXING','GOLF'],
      gradingModes: {
        ...
        sportsbookOutrightOnly: ['GOLF']
      },
      source: 'The Sports Outpost simulations, sportsbook snapshots, and connected live/final result feeds',
      note: 'Soccer, Tennis, MMA and supported Table Tennis matches have connected live/final grading. Table Tennis uses official World Table Tennis data and fails closed on ambiguity or void-like results. Esports, Volleyball, Cricket and Boxing use complete Pinnacle pregame prices. Rugby League/NRL and AFL are pipeline-ready but had no upcoming event sample in the latest validation window. Golf supports only explicit tournament-winner outrights from complete current Pinnacle fields; Golf settlement, leaderboard inference, withdrawals, round props and matchup props are not connected and never infer HIT/MISS. Any missing or ambiguous sportsbook-only selection remains unresolved.'
    },
    parser: {
