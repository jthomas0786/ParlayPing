...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    for(const mention of targetMentions){
      ...
      const raw=await analyzeMultiSport(parsed.legs,{baseUrl,referenceTime:parent.created_at||mention.created_at||null});
      const analysis=applyCorrelationSafety(raw);const readiness=replyReadiness(analysis);
      const share=readiness.ready?tryBuildXShareBundle({parsedLegs:parsed.legs,analysis,sourceReference:`x:${parentId}`,baseUrl,maxReplyLegs:3}):{ready:false,reason:readiness.reason,shareUrl:null,cardUrl:null,xReplyCardUrl:null,replyText:null};
      row.readiness=readiness;row.counts=analysis.counts;row.supportedSports=analysis.supportedSports;row.unsupportedSports=analysis.unsupportedSports;row.correlation=analysis.correlation;row.combinedTailProbability=analysis.combinedTailProbability;row.combinedTailProbabilityMethod=analysis.combinedTailProbabilityMethod;row.tailUrl=analysis.tailUrl||null;row.shareReady=Boolean(share.ready);row.shareReason=share.reason||null;row.shareUrl=share.shareUrl||null;row.cardUrl=share.cardUrl||null;row.xReplyCardUrl=share.xReplyCardUrl||null;row.unresolvedLegs=(analysis.results||[]).filter(r=>r?.status==='UNRESOLVED').map(unresolvedSummary);row.status=readiness.ready?'ready':'needs-match';row.replyPreview=readiness.ready?(share.replyText||buildPublicReply(analysis,{maxLegs:3})):null;rows.push(row);
    }
    return res.status(200).json({ok:true,dryRun:true,authenticatedAs:{id:String(me.data.id),username:me.data.username,name:me.data.name},postingSafety:{approvalRecorded:String(process.env.X_AI_REPLY_APPROVED||'').toLowerCase()==='true',autoReplyEnabled:false},shareCards:{configured:String(process.env.PARLAYPING_SHARE_SECRET||'').length>=32,postingStillDisabled:true},vision:{configured:Boolean(process.env.OPENAI_API_KEY),model:process.env.OPENAI_MODEL||'gpt-5.6-luna'},analysisAdapters:ANALYSIS_ADAPTERS,mentionsSeen:(mentions.data||[]).length,candidates:rows});
