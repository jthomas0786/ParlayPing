...
function pageHtml({ slip, shareUrl, cardUrl, baseUrl, liveDataAvailable }) {
  ...
  const backControl = returnUrl ? `<a class="return-btn" href="${esc(returnUrl)}" aria-label="Back to ${esc(returnLabel)}"><span aria-hidden="true">←</span><span class="return-copy">Back to ${esc(returnLabel)}</span></a>` : '<span></span>';
  const closeControl = returnUrl ? `<a class="close-btn" href="${esc(returnUrl)}" aria-label="Close ParlayPing and return to ${esc(returnLabel)}">×</a>` : '<span></span>';
